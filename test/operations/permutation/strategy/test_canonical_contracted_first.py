"""Tests for the canonical contracted-first permutation strategy."""

import pytest

from memory import Memory
from operations.contraction.path import ContractionPath
from operations.permutation.strategy.canonical_contracted_first import (
...
class TestCanonicalContractedFirstPermutationStrategy:
    """Tests for canonical contracted-first permutation layouts."""

    def test_find_optimal_permutation_single_step(self, single_step_network: TensorNetwork) -> None:
        """The strategy swaps contracted indices to the front on a simple pair."""
        network = single_step_network
        contraction_path: ContractionPath = [(0, 1)]

        initial_perms, intermediate_perms = (
            CanonicalContractedFirstPermutationStrategy.find_optimal_permutation(
                network, contraction_path
            )
        )

        assert initial_perms == [(1, 0), (0, 1)]
        assert intermediate_perms == [(1, 0)]
    ...
    def test_find_optimal_permutation_three_tensor_chain(
        ...
        assert initial_perms == [(1, 0), (0, 1), (0, 1)]
        assert intermediate_perms == [(1, 0), (1, 0)]

    def test_find_optimal_permutation_multiple_contracted_indices(
        self, multi_shared_index_network: TensorNetwork
    ) -> None:
        """The strategy sorts contracted indices by size before free indices."""
        ...
        assert initial_perms == [(1, 2, 0), (0, 2, 1)]
        assert intermediate_perms == [(1, 0)]
...
class TestCanonicalContractedFirstPermutationStrategyMemory:
    """Tests for canonical contracted-first memory estimations."""
    ...
    def test_get_peak_and_total_memory_single_step(
        ...
        peak_memory = CanonicalContractedFirstPermutationStrategy.get_peak_memory(
            network, contraction_path
        )
        total_memory = CanonicalContractedFirstPermutationStrategy.get_total_memory(
            network, contraction_path
        )
