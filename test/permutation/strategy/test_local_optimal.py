...
class TestLocalOptimalPermutationStrategyMemory:
    """Tests for local optimal memory estimations."""

    def test_get_peak_memory_single_step_equals_manual_value(self) -> None:
        """Peak memory is correct for one contraction."""
        network = TensorNetwork(
            input_indices=[[0, 1], [1, 2]],
            size_dict={0: 2, 1: 3, 2: 4},
            shapes=[(2, 3), (3, 4)],
    ...
    def test_get_total_memory_two_step_chain_equals_manual_value(self) -> None:
        ...
        total_memory = LocalOptimalPermutationStrategy.get_total_memory(network, contraction_path)
